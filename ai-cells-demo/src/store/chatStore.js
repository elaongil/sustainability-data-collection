...
const RESPONSE_TEMPLATES = {
  ...
  entitySemantic: 'I noticed {entities} in your message. Based on this context, {analysis}',
  wikipediaEnhanced: 'Based on your request about {entities}, I found these tables from Wikipedia:\n\n\n{tableData}\n\n\nThis information suggests that {analysis}',
};
...
export const useChatStore = create((set, get) => ({
  ...
  processMessage: async (content) => {
    ...
    const hasResponseGenerator = activeProcessors.some(cell => cell.id === 'response-generation');
    const hasWikipediaExtractor = activeProcessors.some(cell => cell.id === 'wikipedia-extractor');
    ...
    const isWikiConnectedToResponse = connections.some(
      conn => conn.source === 'wikipedia-extractor' && conn.target === 'response-generation'
    );
    
    if (!hasResponseGenerator || (hasWikipediaExtractor && !isWikiConnectedToResponse)) {
    ...
    for (const cell of activeProcessors) {
      ...
      await new Promise(resolve => setTimeout(resolve, PROCESSING_DELAYS[cell.id.split('-')[0]]));
      ...
      switch (cell.id) {
        case 'wikipedia-extractor':
          if (isWikipediaUrl(content)) {
            try {
              ...
              if (responseData.data.tables && responseData.data.tables.length > 0) {
                tableData = responseData.data.tables.map(table => {
                  ...
                  const rows = cleanData.map(row => row.join(' | ')).join('\n');
              ...
              if (!analysis) analysis = 'the tables contain structured data from the Wikipedia article';
            } catch (error) {
              console.error('Wikipedia extraction error:', error);
              const errorMessage = error.message || 'Unknown error occurred';
              const technicalDetails = [];
              if (apiResponse?.status) technicalDetails.push(`Status Code: ${apiResponse.status}`);
              if (responseData?.data?.error) technicalDetails.push(`Server Error: ${responseData.data.error}`);
              if (error.stack) technicalDetails.push(`Stack Trace: ${error.stack}`);
        ...
        case 'entity-recognition':
          entities = 'AI models, specifically GPT and BERT series, their developers like OpenAI and Google';
          break;
        case 'semantic-analysis':
          analysis = 'there is a trend of increasing model size and complexity, with a shift from open-source to proprietary licenses';
          break;
        case 'response-generation':
          ...
          if (activeProcessors.length === 1) {
            ...
            if (activeIds.includes('semantic-analysis') || activeIds.includes('entity-recognition')) {
            ...
            const hasEntitySemantic = activeIds.includes('entity-recognition') && activeIds.includes('semantic-analysis');
            ...
            if (activeProcessors.length >= 3) {
              ...
              response = RESPONSE_TEMPLATES.entity.replace('{entities}', entities);
            } else if (activeIds.includes('semantic-analysis')) {
              response = RESPONSE_TEMPLATES.semantic.replace('{analysis}', analysis);
            }
          }
          break;
    ...
    if (!response && activeProcessors.some(cell => cell.id === 'wikipedia-extractor')) {
      response = RESPONSE_TEMPLATES.wikipedia.replace('{tableData}', tableData);
    }
  ...
  clearChat: () => set({ messages: [], isProcessing: false, processingSteps: [] }),
}));
